...
- `./cerumbra-server.sh`  
  ...
  If confidential computing is still disabled, run `sudo ./ccadm-setup.sh` on the DGX Spark node to switch the GPU into SECURE mode and reboot. The helper attempts to install `nvidia-ccadm` automatically via `apt-get` when it is missing.
- `./cerumbra-client.sh`  
