...
- `./cerumbra-server.sh`  
  ...
  If confidential computing is still disabled, run `sudo ./ccadm-setup.sh` on the DGX Spark node to switch the GPU into SECURE mode and reboot. The helper bootstraps NVIDIA's CUDA apt repository (via `cuda-keyring`) as needed and installs `nvidia-ccadm` automatically before enabling secure mode.
- `./cerumbra-client.sh`  
